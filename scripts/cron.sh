...
docker run 933782373565.dkr.ecr.eu-west-1.amazonaws.com/pio:importer-latest `date -d "2 hours ago" "+%Y-%m-%dT%H:%M:%S"Z` `date "+%Y-%m-%dT%H:%M:%S"Z`
...
cd /opt/prediction-io/ && docker-compose restart 
